...
def find_relevant_rank(retrieved_documents, expected_paper):
    for rank, (document, _) in enumerate(retrieved_documents, start=1):
        if document.metadata.get("paper_name") == expected_paper:
            return rank
    return None
...
def evaluate_reranker(
    ...
    for item in evaluation_questions:
        ...
        if relevant_rank is not None:
            ...
            category_stats[category]["rr_sum"] += 1 / relevant_rank

        print(f"\nQuestion: {question}")
        print(f"Category: {category}")
        print(f"Expected Paper: {expected_paper}")

        print("Top Retrieved Papers After Reranking:")
        for rank, (document, score) in enumerate(reranked_documents, start=1):
            print(f"{rank}. {document.metadata.get('paper_name')} ({score:.4f})")
    ...
    return recall_at_k, mrr, category_stats


if __name__ == "__main__":
    print("Loading evaluation questions...")
    evaluation_questions = load_evaluation_questions("evaluation/questions.json")
    ...
    recall_at_k, mrr, category_stats = evaluate_reranker(
        evaluation_questions,
        index,
        bm25,
        chunks,
    ...
    print(f"Recall@{FINAL_TOP_K}       : {recall_at_k:.4f}")
    print(f"MRR                 : {mrr:.4f}")

    print("\n======= Category-wise Results =======")

    for category, stats in category_stats.items():
        category_recall = stats["hits"] / stats["questions"]
        category_mrr = stats["rr_sum"] / stats["questions"]

        print(f"\n{category.title()}")
        print("-" * len(category))
        print(f"Questions : {stats['questions']}")
        print(f"Recall@{FINAL_TOP_K}: {category_recall:.4f}")
